...
import { addTodo, createProject, globalSearch } from '../controllers/projectController.js';
...
api.post('/project', verifyTokenMiddleware,createProject)

// Create a new API endpoint for finding projects
api.get('/findProject', verifyTokenMiddleware, async (req, res) => {
    try {
        // Pagination parameters
        const page = req.query.page || 1;
        const limit = req.query.limit || 10;

        // Construct the query based on provided title and/or description
        const query = {};
        if (req.query.title) {
            query.title = { $regex: new RegExp(req.query.title, 'i') }; // Case-insensitive title search
        }
        if (req.query.description) {
            query.description = { $regex: new RegExp(req.query.description, 'i') }; // Case-insensitive description search
        }
...
api.put('/project/:id', verifyTokenMiddleware, addTodo)
...
api.get('/globleSerach',globalSearch)
